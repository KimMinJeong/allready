...
<body>
		...
		%>
				<center>
					<button type="button" name="button" class="btn btn-default btn-lg"
						onClick="location.href='addGood.do?board_no=<%=vo.getBoard_no()%>'"
					...
					<button type="button" name="button" class="btn btn-default btn-lg"
						onClick="location.href='addBad.do?board_no=<%=vo.getBoard_no()%>'"
						onclick="event.cancelBubble = true">
						...
						<div class="navbar-form navbar-center" role="search">
							<div class="form-group">
								<input type="hidden" class="form-control" placeholder="게시판넘버"
									...
									id="writer_id" value="<%=vo2.getUser_id()%>"> <input
									type="text" class="form-control inline"
									placeholder="댓글을 입력해주세요." size="80" id="contents">
								<button type="button" class="btn btn-default inline"
									id="addComments" value=<%=vo.getBoard_no() %>>입력</button>
								<br>
								<br>
  ...
  %>
												<center>
													<button type="button"
														class="btn btn-default deleteComments"
														value="<%=vc.getComments_no()%>">삭제</button>
												</center> <%} %>
											</td>
